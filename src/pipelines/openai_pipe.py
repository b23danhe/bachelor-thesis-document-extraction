...
def run(file_path_str: str, dataset: str, validation_folder_path: Path, csv_writer: SaveToCSV, output_dir: Path) -> None:
    ...
    if dataset_name == "mio":
        raw_data = call_openai(file_path_str, ZEROSHOT, schema, api_key=settings.openai_api_key, model=settings.openai_model)
    elif dataset_name == "fatura":
        raw_data = call_openai(file_path_str, ZEROSHOTFATURA, schemaFATURA, api_key=settings.openai_api_key, model=settings.openai_model)
    elif dataset_name == "cord":
        raw_data = call_openai(file_path_str, ZEROSHOTCORD, schemaCORD, api_key=settings.openai_api_key, model=settings.openai_model)
